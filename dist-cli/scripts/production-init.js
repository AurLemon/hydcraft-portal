...
const DEFAULT_SCOPES = ['owner', 'links', 'partners'];
const pool = new Pool({
...
const findMatchingFriendLink = async (name, category) => {
...
const ensureFriendLinks = async () => {
    let created = 0;
    let updated = 0;
    for (const linkConfig of productionInitConfig.friendLinks) {
        const existing = await findMatchingFriendLink(linkConfig.name, linkConfig.category);
        if (!existing) {
            ...
            created += 1;
            console.log(`[production-init] friend link created: ${linkConfig.category}/${linkConfig.name}`);
            continue;
        }
        ...
        updated += 1;
        console.log(`[production-init] friend link updated: ${linkConfig.category}/${linkConfig.name}`);
    }
    return { created, updated };
};
const findMatchingPartner = async (name, section) => {
...
const ensurePartners = async () => {
    ...
    for (const partnerConfig of productionInitConfig.partners) {
        const existing = await findMatchingPartner(partnerConfig.name, partnerConfig.section);
        if (!existing) {
            ...
            created += 1;
            console.log(`[production-init] partner created: ${partnerConfig.section}/${partnerConfig.name}`);
            continue;
        }
        ...
        updated += 1;
        console.log(`[production-init] partner updated: ${partnerConfig.section}/${partnerConfig.name}`);
    }
    return { created, updated };
};
const main = async () => {
    ...
    if (options.scopes.has('links')) {
        ...
        console.log(`[production-init] friend links synced: created=${result.created}, updated=${result.updated}`);
    }
    if (options.scopes.has('partners')) {
        const result = await ensurePartners();
        console.log(`[production-init] partners synced: created=${result.created}, updated=${result.updated}`);
    }
};
try {
    await main();
